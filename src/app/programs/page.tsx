      ...
      <section className="pt-32 pb-16 md:pt-40 md:pb-24">
        <div className="mx-auto max-w-7xl px-6 md:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="mb-4 inline-flex liquid-glass rounded-full px-3.5 py-1 text-xs font-medium font-body text-white">Programs</p>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {programs.map((p) => (
              <div key={p.name} className="liquid-glass rounded-2xl p-8 transition-colors">
                <p className="mb-2 text-xs font-medium font-body uppercase tracking-wider text-white/40">{p.tagline}</p>
                <h2 className="mb-3 font-heading italic text-xl tracking-tight text-white">{p.name}</h2>
                <p className="text-sm font-body font-light leading-relaxed text-white/60">{p.description}</p>
              </div>
            ))}
          </div>
          <div className="mt-12 text-center">
            <a href="/request" className="inline-flex rounded-full bg-white px-8 py-3.5 text-sm font-semibold font-body text-black transition-colors hover:bg-white/90">
              Request Information
            </a>
          </div>
        </div>
